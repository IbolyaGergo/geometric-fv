...
def _update_cell_implicit_upwind(idx: int,
                                  u_old: np.ndarray,
                                  u_new: np.ndarray,
                                  cfl: float,
                                  get_bc_func: callable) -> float:
    if idx == 0:
...
_update_cell_schemes = {
    SchemeType.IMPLICIT_UPWIND: _update_cell_implicit_upwind
}
...
def update_cell(idx: int,
                ...
                get_bc_func: callable = None) -> float:
    """
...
def sweep(ncells: int,
          u_old: np.ndarray,
          cfl: float,
          scheme_type: SchemeType = SchemeType.IMPLICIT_UPWIND,
          get_bc_func: callable = None) -> np.ndarray:
    ...
    for i in range(ncells):
        u_new[i] = update_cell(i, u_old, u_new,
                                cfl, scheme_type, get_bc_func)
