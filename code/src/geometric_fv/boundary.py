...
def _apply_bc_constant_extend(u_old: np.ndarray,
                              u_new: np.ndarray,
                              nghost: int):
    # BC
    u_old[0] = u_old[1]
...
#     u[-1] = u[3]
#     u[-2] = u[2]
def _apply_bc_quasi_periodic(u_old: np.ndarray,
                             u_new: np.ndarray,
                             nghost: int):
...
def apply_bc(bc_type: BCType,
    ...
    return apply_bc_func(u_old, u_new, nghost)
