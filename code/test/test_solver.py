...
from geometric_fv.solver import SchemeType, sweep, update_cell
...
def test_update_cell_implicit_upwind_constant_input(val):
    ...
    assert pytest.approx(val) == update_cell(
        1, u_stencil_old, u_stencil_new, 1, scheme_type=SchemeType.IMPLICIT_UPWIND
    )
...
def test_update_cell_implicit_upwind_concrete_values(u_old_i, u_new_im1, expected):
    ...
    assert pytest.approx(expected) == update_cell(
        1, u_stencil_old, u_stencil_new, 1, scheme_type=SchemeType.IMPLICIT_UPWIND
    )
