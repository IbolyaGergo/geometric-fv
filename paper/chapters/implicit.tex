...
The explicit finite volume schemes discussed in the previous chapter are limited by the Courant-Friedrichs-Lewy (CFL) condition, requiring \(\nu \leq 0\).
...
The cost of this remarkable stability property is considerable numerical diffusion. Compared to the explicit upwind scheme, the implicit formulation exhibits significantly higher damping.
...
\subsection{Geometric Interpretation: REA}%
\label{sub:Geometric Interpretation: REA}

We can interpret the implicit upwind scheme within the Reconstruct-Evolve-Average framework.
The implicit case differs in the reconstruction step: we construct a piecewise constant function using the \textit{new} values instead of the old ones:
\[
    \tilde{u}(x,t^n) = u_{i}^{n+1}.
\]
(See Figure \ref{fig:implicit_upwind_reconstruct}).
...
\]
The method effectively reconstructs a "future" profile and asserts that the flux generated by this profile satisfies the conservation law.
