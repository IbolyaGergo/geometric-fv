...
In this section, we derive a general formulation for second-order implicit finite volume schemes.
Following the geometric arguments from the explicit case, we seek a numerical flux derived using a linear reconstruction incorporating both known averages from the old time \(t^n\) and unknown values from \(t^{n+1}\).
All the schemes derived in this work can be viewed as special cases of a single general numerical flux, distinguished only by their definition of the slope \(\sigma_i\).

\subsection{Derivation of the General Implicit Flux}%
\label{sub:derivation_general_flux}

To construct a second-order scheme, we replace the piecewise-constant profile with a piecewise-linear one.
In the implicit case, we define a reconstruction at time level \(t^n\) that utilizes the yet unknown value \(u_{i}^{n+1}\).
Specifically, we define the linear profile such that it passes through the new average \(u_{i}^{n+1}\) at the position \(x_i - a\Delta t\):
\begin{equation}
    \label{eq:general_reconst_implicit}
    \tilde{u}(x, t^n) = u_i^{n+1} + \sigma_i (x - (x_i - a\Delta t)),
\end{equation}
where \(\sigma_i\) is a slope.

The numerical flux \(F_{i+1/2}\) is the time-average of the flux at the interface \(x_{i+1/2}\).
Assuming a positive constant velocity \(a > 0\), the value at the interface \(u(x_{i+1/2}, t)\) for \(t \in [t^n, t^{n+1}]\) is found by:
\begin{equation}
    u(x_{i+1/2}, t) = \tilde{u}(x_{i+1/2} - a(t - t^n), t^n).
\end{equation}
Substituting the reconstruction formula \eqref{eq:general_reconst_implicit} yields:
\begin{equation}
    \begin{split}
    u(x_{i+1/2}, t) &= u_i^{n+1} + \sigma_i \left[ (x_{i+1/2} - a(t - t^n)) - (x_i - a \Delta t) \right] \\
                    &= u_i^{n+1} + \sigma_i \left[ \frac{\Delta x}{2} + a(t^{n+1} - t) \right]. 
    \end{split}
\end{equation}
Integrating this value over the time step $[t^n, t^{n+1}]$ and multiplying by $a$ gives the average flux:
\begin{equation}
    \begin{split}
        F_{i+1/2} &= \frac{a}{\Delta t}
        \int_{t^n}^{t^{n+1}} \left[
    u_i^{n+1} + \sigma_i \left( \frac{\Delta x}{2} + a(t^{n+1} - t) \right)
    \right] \diff t \\
                  &= a u_i^{n+1} + \frac{a \sigma_i \Delta x}{2} + \frac{a^2 \sigma_i \Delta t}{2}.
    \end{split}
\end{equation}
After factoring out terms, recognizing the Courant number \(\nu = a \Delta t / \Delta x\), we arrive at the general second-order implicit flux:
\begin{equation}
    \label{eq:general_implicit_flux}
    F_{i+1/2} = a u_{i}^{n+1} + \frac{1}{2} a \sigma_i \Delta x (1 + \nu).
\end{equation}
This formula serves as the foundation for the high-order schemes discussed in the following subsections.
We will show that the classical Box scheme and the IIOE scheme are simply different geometric choices for the slope \(\sigma_i\).

\subsection{The Box Scheme}%
\label{sub:box_scheme}
...
As we demonstrated in Section \ref{sec:construction_limiters}, in the context of explicit schemes, the Upwind Range Condition (URC) provides a robust geometric constraint for constructing high-resolution methods.
...
In this section, we formulate an \textit{Implicit Upwind Range Condition (IURC)}, an extension of the classical Upwind Range Condition that, to our knowledge, has not been previously formulated.
We demonstrate that the algebraic M-Matrix constraints can be reinterpreted geometrically as an Implicit~Upwind~Range~Condition(IURC).
Furthermore, as shown in Section \ref{sub:implicit_tvd}, any scheme satisfying the IURC is also Total Variation Diminishing (TVD).

The primary motivation for introducing this \textit{local} property is its conceptual simplicity and practical utility compared to global algebraic conditions.
As a geometric constraint, it is more intuitive and more readily adaptable to non-uniform grids or variable velocities 
(see also Thuburn \cite{thuburn1997tvd} who pointed out similar arguments for the explicit case).
This unified perspective allows for the construction of high-resolution implicit schemes in direct analogy to the well-established framework for explicit schemes.

The classical implicit upwind scheme \eqref{eq:implicit_upwind_cons} provides the foundation for our analysis.
Its solution can be expressed as:
\begin{equation}
    \label{eq:implicit_upwind_solution_iurc}
    u_{i}^{n+1} = \frac{1}{1+\nu}u_{i}^{n} + \frac{\nu}{1+\nu}u_{i-1}^{n+1}.
\end{equation}
For any non-negative Courant number, \(\nu \geq 0\), the coefficients form a convex combination, i.e., they are non-negative and sum to unity.
A direct consequence is that the scheme satisfies a local maximum principle, that the new value \(u_{i}^{n+1}\) is bounded by the old value \(u_{i}^{n}\) and the new upwind value \(u_{i-1}^{n+1}\)
\[
    \min(u_{i}^{n},u_{i-1}^{n+1}) \leq u_{i}^{n+1} \leq \max(u_{i}^{n},u_{i-1}^{n+1}).
\]
This property is key.
By analogy with the explicit framework derived in Section \ref{sec:monotonicity_toro}, we can reformulate this boundedness into an equivalent ratio form:
\begin{equation}
    \label{eq:implicit_upwind_range_condition}
    0 \leq \frac{u_{i}^{n+1}-u_{i}^{n}}{u_{i-1}^{n+1}-u_{i}^{n}} \leq 1.
\end{equation}
We define this as the \textit{Implicit~Upwind~Range~Condition}.
This geometric constraint serves as a fundamental building block for constructing high-resolution, oscillation-free implicit schemes, providing a direct analogue to explicit methods.

\subsection{TVD Property}%
\label{sub:implicit_tvd}

A key result is that any scheme satisfying the IURC \eqref{eq:implicit_upwind_range_condition} is also Total Variation Diminishing (TVD).
We demonstrate this by first showing that the IURC implies that a scheme can be written in a form known to be TVD.

The IURC requires that the new cell average \(u_{i}^{n+1}\) lies between the values \(u_{i-1}^{n+1}\) and \(u_{i}^{n}\).
This is equivalent to stating that \(u_{i}^{n+1}\) can be written as a convex combination of the two:
\begin{equation}
    u_{i}^{n+1} = k_i u_{i}^{n} + (1-k_i)u_{i-1}^{n+1},
\end{equation}
for some \(k_i \in [0, 1]\).
Since the condition was derived from the solution of the implicit upwind scheme, for any non-zero \(k_i \neq 0 \), this expression can be recast into the conservative form of the first-order implicit upwind scheme.
By rearranging the equation above, we get:
\begin{equation}
    u_{i}^{n+1} = u_{i}^{n} - \frac{1-k_i}{k_i} \left( u_{i}^{n+1} - u_{i-1}^{n+1} \right).
\end{equation}
If we define \(c_i = (1-k_i)/k_i\), we see that \(c_i \ge 0\).
The scheme now has the general form of the implicit upwind method, which is sufficient for a scheme to be TVD, see, e.g.,~\cite{duraisamy2007implicit,frolkovic2023high}.

If \(k_{i} = 0\), then
\[
    u_{i}^{n+1} = u_{i-1}^{n+1},
\]
thus, no new extrema is created.

\subsection{M-Matrix Property}%
\label{sub:implicit_m_matrix}
...
\subsection{Analysis of the Box Scheme}
\label{sec:box_scheme_iurc_analysis}

A next step is to determine if the unmodified second-order box scheme satisfies the IURC.
First, we solve the box scheme equation \eqref{eq:second_order_implicit_matrix} for \(u_i^{n+1}\):
\begin{align*}
    (1+\nu)u_{i}^{n+1} &= (1+\nu)u_{i-1}^{n} + (1-\nu)u_i^{n} - (1-\nu)u_{i-1}^{n+1} \\
    u_i^{n+1} &= u_{i-1}^{n} + \frac{1-\nu}{1+\nu}u_i^{n} - \frac{1-\nu}{1+\nu}u_{i-1}^{n+1}\\
              &= u_{i-1}^{n} + \frac{1-\nu}{1+\nu}\left( u_i^{n} - u_{i-1}^{n+1} \right)
\end{align*}
Next, we substitute this into the IURC ratio, \(\frac{u_{i}^{n+1}-u_{i}^{n}}{u_{i-1}^{n+1}-u_{i}^{n}}\). The numerator becomes:
\begin{equation*}
    u_i^{n+1} - u_i^n = \frac{1-\nu}{1+\nu}\left( u_i^{n} - u_{i-1}^{n+1} \right) - \left( u_i^n - u_{i-1}^{n}  \right).
\end{equation*}
Thus, the full expression for the ratio:
\[
    \frac{u_{i}^{n+1}-u_{i}^{n}}{u_{i-1}^{n+1}-u_{i}^{n}} = 
    -\frac{1-\nu}{1+\nu} + \frac{u_{i}^{n}-u_{i-1}^{n}}{u_{i}^{n}-u_{i-1}^{n+1}}.
\]
The expression is clearly not a constant value; it depends on the solution profile itself.
This proves that the IURC is not satisfied for arbitrary data.

Despite its unconditional stability and second-order accuracy, the box scheme is not guaranteed to be monotonic.
This provides motivation for developing a limited version of the scheme, which we will address in the following section.
...
To remedy this, we now construct a high-resolution version of the scheme.
...
\subsection{General Limiters}%
\label{sub:implicit_limiter_general}

To construct high-resolution schemes that remain monotonic, we often use slope limiters.
Many limiters are constructed to mimic the behavior of keeping the solution within the bounds of its neighbors, a property that can be elegantly enforced using a median function.
Before discussing specific limiters, we define the \texttt{minmod} and \texttt{median} functions.
The \texttt{minmod} function is defined as:
\[
    \text{minmod}(a, b) = \frac{1}{2}(\text{sgn}(a) + \text{sgn}(b))\min(|a|, |b|).
\]
It selects the argument with the smallest absolute value if both have the same sign, and returns zero otherwise.
The median of three values can be expressed using the \texttt{minmod} function as:
\[
    \text{median}(x, y, z) = x + \text{minmod}(y-x, z-x).
\]

Now, let's consider a general high-order implicit flux written as a correction to the first-order implicit upwind flux:
\[
    F_{i+1/2}^{\text{lim}} = a \left[ u_{i}^{n+1} + \delta_{i+1/2}^{\text{lim}} \right].
\]
Here, \(a \delta_{i+1/2}^{\text{lim}}\) is the limited high-order correction term. Substituting this into the finite volume update formula, \(u_i^{n+1} = u_i^n - (\Delta t/\Delta x)(F_{i+1/2} - F_{i-1/2})\), yields:
\[
    u_i^{n+1} = u_i^n - \nu \left( (u_{i}^{n+1} - u_{i-1}^{n+1}) + (\delta_{i+1/2}^{\text{lim}} - \delta_{i-1/2}^{\text{lim}}) \right).
\]
To satisfy the IURC, we derive a general constraint on the corrections.
Rearranging the equation gives:
\begin{equation*}
    \left( u_{i}^{n+1}-u_{i}^{n} \right) (1+\nu)
    = \nu \left( u_{i-1}^{n+1}-u_{i}^{n} \right) -\nu \left( \delta_{i+1/2}^{\text{lim}} - \delta_{i-1/2}^{\text{lim}} \right).
\end{equation*}
Provided \(u_{i-1}^{n+1}-u_{i}^{n} \neq 0\), we can write the IURC ratio as:
\begin{equation*}
    \frac{u_{i}^{n+1}-u_{i}^{n}}{u_{i-1}^{n+1}-u_{i}^{n}}
    = \frac{\nu}{1+\nu} - \frac{\nu}{1+\nu} \frac{ \delta_{i+1/2}^{\text{lim}} - \delta_{i-1/2}^{\text{lim}} }{u_{i-1}^{n+1}-u_{i}^{n}}.
\end{equation*}
The IURC requires this ratio to be in \([0, 1]\).
While both bounds are necessary to guarantee strict monotonicity, the lower bound, \(0 \le \frac{u_{i}^{n+1}-u_{i}^{n}}{u_{i-1}^{n+1}-u_{i}^{n}}\), is considered the primary constraint for several reasons.
First, it directly prevents the formation of new local minima (undershoots), which is a common failure mode for many unlimited high-order schemes, particularly near sharp gradients.
The condition derived from this bound,
\[
    \delta_{i+1/2}^{\text{lim}} - \delta_{i-1/2}^{\text{lim}} \le u_{i-1}^{n+1} - u_i^n,
\]
has a clear physical interpretation: the net anti-diffusive correction across a cell cannot exceed the available "room" for the update, defined by the difference between the upwind state and the cell's current state.
In contrast, the upper bound, \(\frac{u_{i}^{n+1}-u_{i}^{n}}{u_{i-1}^{n+1}-u_{i}^{n}} \le 1\), which prevents overshoots, leads to the condition \(-1/\nu \le \frac{\delta_{i+1/2}^{\text{lim}} - \delta_{i-1/2}^{\text{lim}}}{u_{i-1}^{n+1}-u_{i}^{n}}\).
This second constraint is dependent on the Courant number and is often automatically satisfied by standard limiter formulations once the more restrictive lower bound is met.
Thus, ensuring the lower-bound condition is the crucial step in suppressing the most prominent oscillations of the underlying high-order scheme.
This primary condition is analogous to the constraints for explicit limiters, but relates the correction terms to the solution at the new time level.
Specific limiter functions, such as van Leer's, can be adapted to this implicit framework to define the correction \(\delta_{i+1/2}^{\text{lim}}\) in a way that satisfies this condition.

While the limiters are expressed via the median function for theoretical clarity, they are implemented using hardware-optimized min/max operations to avoid branching and floating-point divisions, ensuring high computational efficiency.
