...
\begin{figure}[ht]
    ...
    \caption{(Top) Assumed reconstruction at time \(t^{n}\) used for estimating the fluxes in the implicit upwind scheme \eqref{eq:implicit_upwind_cons}.
    (Bottom) Numerical flux is the area swept across the interfaces (red) during the evolution.}
...
To overcome the low accuracy and high numerical diffusion of this approach, we must develop a higher-order scheme.
Following the logic from the explicit framework, this requires a more sophisticated, piecewise-linear reconstruction of the data.
In the next section, we construct a second-order accurate implicit scheme by defining a linear reconstruction that is consistent with the cell average data at time \(t^n\).

\section{Second-Order implicit schemes}%
\label{sec:second_order_implicit}

% TODO: Add reconstruction and flux fig
% Caption: (Top) The consistent, piecewise-linear reconstruction at time \(t^n\).
% The slope \(\sigma_i\) is chosen such that the line passes through the back-traced new average \(u_i^{n+1}\).
% (Bottom) The corresponding numerical flux.
