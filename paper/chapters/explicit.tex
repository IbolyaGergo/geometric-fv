...
\label{par:REA}
Let \(u_i^n\) denote the cell average over the \(i\)-th cell at time \(t_n\).
The update to \(t_{n+1} = t_n + \Delta t\) proceeds in three steps:
...
Geometrically, the entire staircase shifts to the right by a distance \(a\Delta t\).
Assuming the CFL condition \(a\Delta t \leq \Delta x\), the solution at the interface \(x_{i-1/2}\) is determined by the value of the cell to the left, \(u_{i-1}^n\).

\textbf{3. Average:}
Computing the new cell average by directly integrating the evolved profile \(\tilde{u}\) is mathematically equivalent to a flux-based update.
...
However, due to the non-zero slope, the geometric shape swept across the boundary is no longer a rectangle, but a \textbf{trapezoid}.
...
A numerical scheme is said to be \textbf{Monotonicity Preserving (MP)} if it maintains the monotonic nature of the initial data.
Formally, if the data at time \(t_n\) is monotonic (i.e., \(u_i^n \geq u_{i+1}^n\) for all \(i\), or \(u_i^n \leq u_{i+1}^n\) for all \(i\)), then the updated solution at \(t_{n+1}\) must satisfy the same monotonicity property.
...
Since the physical solution preserves monotonicity, the updated average \(u_i^{n+1}\) in the \(i\)-th cell should logically be bounded by the current value \(u_i^n\) and the upwind neighbor \(u_{i-1}^n\).
This physical intuition holds strictly under the CFL condition:
\[
    a \Delta t \leq \Delta x \implies \nu \leq 1.
\]

This requirement is formulated as the \textbf{Upwind Range Condition}.
For the scheme to be monotonic and physically bounded, the updated value must lie between the old cell average and the upwind neighbor:
\begin{equation}
    \min(u_{i-1}^n, u_i^n) \leq u_i^{n+1} \leq \max(u_{i-1}^n, u_i^n).
\end{equation}
This condition can be expressed more precisely as a constraint on the ratio of the solution change.
Rearranging terms, we require that the normalized update satisfies:
\begin{equation}
    \label{eq:upwind_range_condition_explicit}
    0 \leq \frac{u_i^{n+1} - u_i^n}{u_{i-1}^n - u_i^n} \leq 1.
\end{equation}
The denominator \(u_{i-1}^n - u_i^n\) represents the maximum possible physical inflow from the upwind cell, while the numerator \(u_i^{n+1} -
u_i^n\) represents the actual numerical update.

\paragraph*{Example: Godunov Scheme}
Substituting the Godunov scheme \eqref{eq:godunov_scheme} into \eqref{eq:upwind_range_condition_explicit}, we obtain:
\[
    0 \leq
