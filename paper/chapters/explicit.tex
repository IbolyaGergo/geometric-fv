...
% TODO: Actually, Van Leer interpreted it as an REA
...
This physical requirement is formalized by Laney \cite{laney1998computational} as the \textit{Upwind Range Condition}.
...
\subsection{The Limited Slope and Flux}
We achieve this by introducing a slope limiter function \(\Psi(r_i)\), which depends on the local smoothness monitor \(r_i\).
We define the effective slope \(\sigma_i\) as a modification of a standard high-order reference slope \(\sigma_i^{HO}\):
\begin{equation}
    \label{eq:limited_slope}
    \sigma_i = \Psi(r_i) \, \sigma_i^{HO}.
\end{equation}
A standard choice for the reference slope is the downwind difference (corresponding to the Lax-Wendroff scheme):
\[
    \sigma_i^{HO} = \frac{u_{i+1}^n - u_i^n}{\Delta x}.
\]
Substituting this limited slope into the general linear flux equation derived in Section \ref{sub:second_order} (Eq. \ref{eq:linear_flux}), we obtain the general \textit{High-Resolution Flux}:
\begin{equation}
    \label{eq:flux_limited}
    F_{i+1/2} = a u_i^n + \frac{1}{2} a (1 - \nu) \Delta x \left( \Psi(r_i) \frac{u_{i+1}^n - u_i^n}{\Delta x} \right).
\end{equation}
Simplifying the terms, this can be viewed as the first-order Godunov flux plus a limited anti-diffusive correction:
\[
    F_{i+1/2} = \underbrace{a u_i^n}_{F_{Godunov}} + \underbrace{\frac{1}{2} a (1 - \nu) (u_{i+1}^n - u_i^n) \Psi(r_i)}_{Correction}.
\]

\subsection{Deriving the Limiter Constraints}
We now determine the necessary constraints on the function \(\Psi(r_i)\) such that the resulting scheme satisfies the Upwind Range Condition \eqref{eq:upwind_range_condition_explicit}.

Substituting the limited flux into the conservation law, the normalized update ratio becomes:
\[
    \text{Ratio} = \frac{u_i^{n+1} - u_i^n}{u_{i-1}^n - u_i^n} = \nu + \frac{1}{2}\nu(1-\nu) \left[ \Psi(r_{i-1}) - \Psi(r_i) r_i \right].
\]
For the scheme to be monotonic, this ratio must lie within the interval \([0, 1]\).
This inequality must hold for any Courant number \(0 \leq \nu \leq 1\).
This leads to the following sufficient conditions on the limiter \(\Psi(r)\):

\begin{enumerate}
    \item \textit{Positive Damping:} For \(r \leq 0\) (which indicates a local extremum), we must set \(\Psi(r) = 0\).
        This forces the slope to zero, reverting the scheme to the first-order Godunov method to prevent the creation of false extrema.

    \item \textit{TVD Region:} For \(r > 0\) (smooth monotonic data), the function \(\Psi(r)\) must be bounded.
        A sufficient algebraic condition for satisfying the Upwind Range Condition is:
    \begin{equation}
        \label{eq:tvd_region}
        0 \leq \Psi(r) \leq \min(2, 2r).
    \end{equation}
\end{enumerate}

Any limiter function \(\Psi(r)\) that stays within the region defined by \eqref{eq:tvd_region} is guaranteed to produce a scheme that is second-order accurate in smooth regions (where \(\Psi \approx 1\)) and oscillation-free near discontinuities.
This region is famously known as the \textit{TVD Region} or the \textit{Sweby Diagram} \cite{sweby1984high}.
